...
def upload_to_produits(instance, filename):
    """Génère un chemin unique par commerçant"""
    ext = filename.split('.')[-1]
    unique_name = f"{instance.id}_{int(timezone.now().timestamp())}.{ext}"
    return f"produits/commercant_{instance.commercant.id}/{unique_name}"

def upload_to_boutiques(instance, filename):
    """Génère un chemin unique pour les photos de boutique"""
    ext = filename.split('.')[-1]
    unique_name = f"boutique_{int(timezone.now().timestamp())}.{ext}"
    return f"boutiques/commercant_{instance.user.id}/{unique_name}"
...
class Commercant(models.Model):
    ...
    photo_boutique = models.ImageField(upload_to=upload_to_boutiques, blank=True, null=True, verbose_name="Photo de la boutique")
...
class Produit(models.Model):
    ...
    photo = models.ImageField(upload_to=upload_to_produits, blank=True, null=True, verbose_name="Photo du produit")
    ...
    commercant = models.ForeignKey(
        Commercant, 
        on_delete=models.CASCADE, 
        related_name='produits_commercant'
    )
...
class Promotion(models.Model):
    # ⚠️ CORRECTION : Mettre à jour le related_name ici aussi
    produit = models.ForeignKey(
        Produit, 
        on_delete=models.CASCADE, 
        related_name='promotions_produit'  # ⚠️ Changé de 'promotions' à 'promotions_produit'
    )
