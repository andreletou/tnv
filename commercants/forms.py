...
class CommercantInscriptionForm(UserCreationForm):
    ...
    def save(self, commit=True):
        ...
        user.type_utilisateur = 'commercant'
        user.is_staff = True
        ...
        if commit:
            ...
            # Générer les produits automatiquement basés sur la catégorie
            self.generer_produits_automatiques(commercant)
        
        return user
    
    def generer_produits_automatiques(self, commercant):
        """Génère automatiquement des produits basés sur la catégorie choisie"""
        try:
            # Charger les données du fichier product.json
            product_json_path = os.path.join(settings.BASE_DIR, 'static', 'data', 'product.json')
            
            with open(product_json_path, 'r', encoding='utf-8') as file:
                data = json.load(file)
            
            # Mapping des catégories entre le formulaire et le JSON
            categorie_mapping = {
            ...
            categorie_commercant = self.cleaned_data['categorie']
            categorie_json = categorie_mapping.get(categorie_commercant)
            
            if not categorie_json:
                print(f"Catégorie non trouvée dans le mapping: {categorie_commercant}")
                return
            
            # Trouver la catégorie correspondante dans le JSON
            for categorie_data in data.get('categories', []):
                if categorie_data.get('name') == categorie_json:
                    # Générer les produits pour chaque sous-catégorie
                    for subcategory in categorie_data.get('subcategories', []):
                        for product_data in subcategory.get('products', []):
                            # Créer le produit dans la base de données
                            Produit.objects.create(
                                nom=product_data['name'],
                                description=f"Produit {product_data['name']} - {subcategory['name']}",
                                prix=product_data['price'],
                                stock=product_data['stock'],
                                stock_min=5,
                                categorie=self.map_categorie_produit(categorie_commercant),
                                est_actif=True,
            ...
            print(f"Produits générés automatiquement pour la catégorie: {categorie_commercant}")
            
        except Exception as e:
            print(f"Erreur lors de la génération automatique des produits: {str(e)}")
    
    def map_categorie_produit(self, categorie_commercant):
        """Mappe la catégorie du commerçant vers la catégorie produit"""
        mapping = {
        ...
        return mapping.get(categorie_commercant, 'autre')
...
# Ajouter à la fin de commercants/forms.py
from django import forms
from decimal import Decimal
from clients.models import RetraitCommercant, Portefeuille

class DemandeRetraitForm(forms.ModelForm):
    """Formulaire pour demande de retrait"""
    class Meta:
        model = RetraitCommercant
        fields = ['montant', 'methode', 'numero_compte', 'operateur', 'nom_beneficiaire']
        widgets = {
            'montant': forms.NumberInput(attrs={'class': 'form-control', 'min': '500'}),
            'methode': forms.Select(attrs={'class': 'form-control'}),
            'numero_compte': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Numéro de téléphone ou compte'}),
            'operateur': forms.Select(attrs={'class': 'form-control'}),
            'nom_beneficiaire': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Nom du bénéficiaire'}),
        }
    
    def __init__(self, *args, **kwargs):
        self.portefeuille = kwargs.pop('portefeuille', None)
        super().__init__(*args, **kwargs)
    
    def clean_montant(self):
        montant = self.cleaned_data.get('montant')
        if self.portefeuille and montant:
            # Vérifier que le solde est suffisant (incluant les frais)
            frais = montant * Decimal('0.02')
            total = montant + frais
            
            if self.portefeuille.solde < total:
                raise forms.ValidationError(
                    f"Solde insuffisant. Solde disponible: {self.portefeuille.solde} FCFA, "
                    f"Montant demandé: {montant} FCFA, Frais: {frais} FCFA"
                )
        return montant