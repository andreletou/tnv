...
from .models import Livreur, Livraison, EvaluationLivreur, TourneeLivraison, AlgorithmeOptimisation
...
class TourneeCreationForm(forms.ModelForm):
    """Formulaire pour créer une tournée de livraisons"""
    class Meta:
        model = TourneeLivraison
        fields = ['livreur', 'livraisons']
        widgets = {
            'livraisons': forms.SelectMultiple(attrs={
                'class': 'form-control select2-multiple',
                'data-placeholder': 'Sélectionnez les livraisons'
            })
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['livraisons'].queryset = Livraison.objects.filter(
            statut='acceptee',
            tournees__isnull=True
        )

class OptimisationParametresForm(forms.Form):
    """Formulaire pour paramétrer l'optimisation"""
    algorithme = forms.ModelChoiceField(
        queryset=AlgorithmeOptimisation.objects.filter(est_actif=True),
        label="Algorithme d'optimisation"
    )
    prioriser_temps = forms.BooleanField(
        required=False,
        initial=True,
        label="Prioriser le temps de livraison"
    )
    prioriser_distance = forms.BooleanField(
        required=False,
        initial=False,
        label="Prioriser la distance"
    )
    considerer_trafic = forms.BooleanField(
        required=False,
        initial=True,
        label="Prendre en compte le trafic"
    )
    limite_temps_max = forms.IntegerField(
        min_value=30,
        max_value=480,
        initial=240,
        label="Temps maximum par tournée (minutes)"
    )
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        for field in self.fields:
            if isinstance(self.fields[field].widget, (forms.Select, forms.CheckboxInput)):
                self.fields[field].widget.attrs.update({'class': 'form-control'})

class VehiculeConfigForm(forms.ModelForm):
    """Formulaire de configuration du véhicule"""
    class Meta:
        model = Livreur
        fields = [
            'type_vehicule_obj', 'kilometrage_vehicule', 
            'date_dernier_entretien', 'niveau_batterie',
            'competences_speciales'
        ]
        widgets = {
            'type_vehicule_obj': forms.Select(attrs={'class': 'form-control'}),
            'kilometrage_vehicule': forms.NumberInput(attrs={'class': 'form-control', 'step': '0.1'}),
            'date_dernier_entretien': forms.DateInput(attrs={'class': 'form-control', 'type': 'date'}),
            'niveau_batterie': forms.NumberInput(attrs={'class': 'form-control', 'min': '0', 'max': '100'}),
            'competences_speciales': forms.SelectMultiple(attrs={'class': 'form-control'}),
        }
    
    competences_speciales = forms.MultipleChoiceField(
        choices=[
            ('fragile', 'Colis fragile'),
            ('refrigere', 'Produits réfrigérés'),
            ('volumineux', 'Colis volumineux'),
            ('dangeroux', 'Marchandises dangereuses'),
